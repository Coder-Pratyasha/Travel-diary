...
const ImageSelector = ({image,setImage,handleDeleteImage}) => {
  ...
  return (
    <div>
      ...
      className="hidden"/>
      {!image ? <button className="w-full h-[220px] flex flex-col items-center justify-center bg-slate-50 rounded-sm border border-slate-200/50" onClick={()=>onChooseFile()}>
      <div className="w-14 h-14 flex items-center justify-center bg-cyan-100 rounded-full border border-cyan-100">
        <FaFileUpload className="text-4xl font-bold text-cyan-500"/>
      </div>
